...
export async function insertSellOrder(data: any) {
  ...
  /*
  {"walletAddress":"0xc6F48f56C5Da5c674746C298A460A2E1427d0998","opwAmount":10,"fietAmount":15,"fietCurrency":"USD","rate":1.5,"payment":{"method":"Bank"},"privateSale":false}
  */

  if (!data.walletAddress || !data.opwAmount || !data.fietAmount || !data.fietCurrency || !data.rate || !data.payment) {
    return null;
  }
...
export async function insertBuyOrder(data: any) {
  ...
  if (!data.walletAddress || !data.opwAmount || !data.krwAmount || !data.rate) {
    return null;
  }
